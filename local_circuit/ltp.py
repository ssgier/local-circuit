...
def update_ltp_weights(
    ltp_weights: np.ndarray, spike_steps: np.ndarray, circuit_config: CircuitConfig
) -> None:
    ...
    ltp_weights[:, update_mask] += np.where(
        row[:, update_mask] < col,
        circuit_config.ltp_step_up,
        -circuit_config.ltp_step_down,
    )

    ltp_weights[:, update_mask] = np.clip(ltp_weights[:, update_mask], 0.0, 1.0)
